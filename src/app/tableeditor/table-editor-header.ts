...
export class TableEditorHeader implements OnInit {
  ...
  importCohortDiseaseData(): void {
    ...
    if (cohort.cohortType != 'mendelian') {
      this.notificationService.showError(
        `External ETL only available for mendelian but you tried ${cohort.cohortType}`,
      );
      return;
    }
    if (cohort.diseaseList.length != 1) {
      this.notificationService.showError(
        `External ETL only available for mendelian but you had ${cohort.diseaseList.length} DiseaseData objects`,
      );
      return;
    }
  ...
  async loadExcel(rowBased: boolean = false) {
    ...
    if (!diseaseData) {
      this.notificationService.showError(
        'Could not retrieve disease data. Have you loaded a cohort?',
      );
      return;
    }
    try {
  ...
  async saveExternalTemplateJson(): Promise<void> {
    ...
    if (!etlDto) {
      this.notificationService.showError(
        'Could not save JSON because data table is not initialized',
      );
      return;
    }
    if (etlDto.disease == null) {
      this.notificationService.showError(
        'Could not save JSON because disease data is not initialized',
      );
      return;
    }
  ...
  private validateDiseaseContext(dto: EtlDto): void {
    ...
    if (currentDisease) {
      ...
      this.notificationService.showWarning(
        'Missing disease data. Create the cohort and reload the ETL file',
      );
    }
  }
  ...
  async addToCohortData(): Promise<void> {
    ...
    if (dto == null) {
      this.notificationService.showError(
        'Could not create CohortData because etlDto was not initialized',
      );
      return;
    }
    ...
    try {
      ...
      if (cohort_previous && cohort_previous.rows.length > 0) {
        const merged_cohort = await this.configService.mergeCohortData(
          cohort_previous,
          cohort_dto_new,
        );
